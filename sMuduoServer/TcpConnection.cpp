...
TcpConnection::TcpConnection(EventLoop* loop,int fd,const struct sockaddr_in &client_addr)
:   loop_(loop),
    spchannel_(new Channel()),
    fd_(fd),
    clientaddr_(client_addr),
    halfclose_(false),
    disconnected_(false),
    ...
    bufferout_()
{
    ...
    spchannel_->setReadEventCallback(std::bind(&TcpConnection::HandleRead,this));
    spchannel_->setWriteEventCallback(std::bind(&TcpConnection::HandleWrite,this));
    spchannel_->setCloseEventCallback(std::bind(&TcpConnection::HandleClose,this));
...
void TcpConnection::AddChannelToloop()
{
	...
	//多线程下，加入loop的任务队列
	//主线程直接执行
    //loop_->AddChannelToPoller(spchannel_.get());
    loop_->AddFunctor(std::bind(&EventLoop::AddChannelToPoller,loop_,spchannel_.get()));
}
...
void  TcpConnection::Send(const std::string &s){
    ...
    else
    {
        ...
        loop_->AddFunctor(std::bind(&TcpConnection::SendInLoop,shared_from_this()));//跨线程调用,加入IO线程的任务队列，唤醒
    }
}
...
void TcpConnection::SendInLoop()
{
    ...
    if(res>0){
        ...
        if(bufferout_.size()>0)
        {
            ...
            loop_->UpdateChannelInPoller(spchannel_.get());
        }
        else
        {
           //数据已发完
...
void TcpConnection::Shutdown()
{
    ...
    {
        //不是IO线程，则是跨线程调用,加入IO线程的任务队列,唤醒IO线程去执行
        loop_->AddFunctor(std::bind(&TcpConnection::ShutdownInLoop,shared_from_this()));
    }
}
...
void TcpConnection::ShutdownInLoop()
{
    ...
    loop_->AddFunctor(std::bind(connectionCleanUp_,shared_from_this()));//自己不能清理自己，交给loop执行，Tcpserver清理TcpConnection
	disconnected_ = true;
}
...
void TcpConnection::HandleError()
{
    ...
    //连接标记为清理
    //task添加
    loop_->AddFunctor(std::bind(connectionCleanUp_,shared_from_this()));//自己不能清理自己，交给loop执行，Tcpserver清理
    disconnected_=true;
}
...
void TcpConnection::HandleClose()
{
	...
	//连接标记为清理
	//task添加
	//loop_->AddFunctor(std::bind(connectionCleanUp_,shared_from_this()));
	//closecallback_(this);
